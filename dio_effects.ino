...
void set_color(byte flag) {
  switch(flag){
    ...
    case 1:{
          analogWrite(mass_led[0], led_color[0]);
          analogWrite(mass_led[1], led_color[1]);
          analogWrite(mass_led[2], led_color[2]);
          break;
    }
    default:{
...
void rainbow_fade(uint8_t pause) {
  if (millis() - last_step > pause) {
    ...
    if (c <= 1) {
      ...
      led_color[2] = short(ihue * bright_lvl);
      analogWrite( mass_led[c], led_color[0]);
      analogWrite( mass_led[0], led_color[2]);
    }
    if (ihue == 255)c++;
    last_step = millis();
  }
